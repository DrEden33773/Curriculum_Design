...
void account_info_manager::delete_a_user()
{
    ...
    if (if_continue_func) {
        ...
        cin >> flag;
        if (flag == 'y' || flag == 'Y') {
            // account_info 和 user_list 中，同时删除用户
...
{
    // 全新的逻辑
    fstream temp_account_info(temp_account_info_file_location_, ios::out | ios::binary);
    ...
    while (account_info.read(reinterpret_cast<char*>(cache), sizeof(account_manager::account_info))) {
        if (strcmp(cache->account_str, input_user.c_str()) != 0) {
            temp_account_info.write(reinterpret_cast<char*>(cache), sizeof(account_manager::account_info));
        }
    }
    account_info.close();
...
{
    // 全新的逻辑
    fstream temp_user_list(temp_user_list_file_location_, ios::out | ios::binary);
...
void account_info_manager::change_specific_user_information()
{
    ...
    while (true) {
        ...
        cin >> temp_user_name;
        if (!if_input_user_exists(temp_user_name, 3, false)) {
            cout << temp_user_name << " 这一用户不存在，或是输入了刚刚完成注册、但是没有绑定个人信息的新用户！" << endl;
            cout << "若是第二种情况，请退出程序，随后登录 " << temp_user_name << " 从而完成个人信息的绑定！" << endl;
            cout << "若想重新输入信息，请输入Y/y，否则输入其他字符 >> ";
            string flag;
            cin >> flag;
            if (flag == "y" || flag == "Y") {
...
void account_info_manager::shell_of_account_info_manager()
{
    ...
    while (true) {
        ...
        cout << "\t"
             << "1.展示所有的 用户信息" << endl;
        cout << "\t"
             << "2.模糊搜索 用户信息 并展示 (支持用 学号/姓名/用户名 搜索)" << endl;
        cout << "\t"
             << "3.精确搜索 用户信息 并展示 (支持用 学号/姓名/用户名 搜索)" << endl;
        cout << "\t"
             << "4.修改 某个特定用户的 特定信息 (支持修改 密码/学号/姓名/宿舍号)" << endl;
        cout << "\t"
             << "5.删除 某个特定用户的 所有信息 {注销特定用户}" << endl;
        cout << "\t"
             << "6.再次显示 [可用功能列表]" << endl;
        cout << "\t"
             << "7.返回 [管理员界面]" << endl;
