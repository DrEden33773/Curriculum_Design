...
void NAT_admin::show_if_user_has_unfinished_nat_check()
{
    ...
    */
    cout << "出于时间原因和准确性考量，现在只提供 学号精确搜索 模式！" << endl;
    cout << endl;
    if_user_has_unfinished_nat_check_by_sno();
}
...
void NAT_admin::show_nat_check_suggested_user(const bool& if_generate_file)
{
    ...
    // next
    cout << "按下任意键后，展示 <生成的名单> 的 {预览}，并开始生成名单 ... " << endl;
    _getch();
    Sleep(250);
    // TODO: print
    cout << endl;
    ...
    cout << "=====================================================================" << endl;
    cout << endl;
    cout << "输入Y/y后，便会生成名单，输入其他内容则会跳过生成名单，直接回到上一级菜单 >> ";
    string if_continue_flag;
    cin >> if_continue_flag;
    if (!(if_continue_flag == "Y" || if_continue_flag == "y")) {
        return;
    }
    // TODO: generate_file
    ...
    time_manager today;
    generated_file << endl;
    generated_file << "本文件生成于 " << today << endl;
    generated_file << endl;
