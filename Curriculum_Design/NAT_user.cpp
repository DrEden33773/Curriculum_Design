...
void NAT_user::set_detail_file_path()
{
    ...
    const string file_path = folder_path + sno_ + R"(_)" + name_ + ".dat";
...
{
    const string folder_path = NAT_detail_file_root + R"(\)" + sno_ + R"(_)" + name_ + R"(_detail_info\)";
    const string file_path = folder_path + sno_ + R"(_)" + name_ + R"(_temp)" + ".dat";
...
void NAT_user::confirm_unfinished_tasks_in_detail_file(const vector<time_manager>& confirm_index_list) const
{
    ...
    detail_file.close();
    detail_file.open(current_detail_file_name_, ios::in | ios::binary | ios::out);
    if (!detail_file.is_open()) {
        cout << "打开 personal_NAT_list 失败！ 即将退出程序 ... " << endl;
        Sleep(250);
        exit(-1);
    ...
    rename(current_detail_file_name_.c_str(), temp_current_detail_file_name_.c_str());
}
    ...
    if (!if_multi_confirm_func) {
        ...
        if (partial_confirm_as_finished) {
            ...
            for (;;) {
                ...
                if (input < 0) {
                    break;
                } else if (input <= list_of_unfinished_nat_checks_former_than_today.size() - 1) {
