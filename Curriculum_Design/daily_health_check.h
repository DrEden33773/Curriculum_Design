...
protected:
    ...
    /// function
    inline void make_sure_file_exists() const;
    bool check_if_file_is_empty();
    void make_sure_dictionary_exists() const;
    bool check_if_dictionary_is_empty();
    daily_health_check(); // 空构造函数
    daily_health_check(const string&); // 真·构造函数
    ~daily_health_check();
    inline void var_initializer();
    /// plug_function
};