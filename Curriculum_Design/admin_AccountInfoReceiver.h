...
class admin_info_receiver : public account_manager { // 进行继承，避免主函数中传递参数
protected:
    ...
    auto if_profession_type_valid(int) -> bool; // 学院标识 是否有效 -> true为有效
    inline void make_sure_admin_list_file_exists();
    auto if_admin_list_file_empty() -> bool;

public:
    //构造函数
    admin_info_receiver(); // 真正用得到
    admin_info_receiver(const string&, const string&); // [冗余]
    //最优先执行的函数
